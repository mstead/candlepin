...
describe 'Import Virt Limit Change', :serial => true do
  ...
  it 'allows change to virt limit subscriptions' do
    # check starting quantity of derived pool
    bonus_pool = @cp.list_pools(:owner => @import_owner.id, :product => 'virt-limit-prov-prod')[0]
    ...
    Dir.entries(products_dir).reject{|entry| entry == "." || entry == ".."}.each do |file|
        ...
        if file.end_with? "virt-limit-tester.json"
